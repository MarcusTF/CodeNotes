...
function promiseContainScope() {
    ...
    const nameOfYourPromise = (                                       //declare a variable and set it equal to a "new Promise()".
        ...
        setTimeout(() => {                                  //settimeout is used here to simulate a process taking some time to execute.
            if (success) {                                  //if success = true, resolve() is called, setting the promise to "fulfilled"
                let successResult = '"success state"'       //anything passed into resolve will be passed into the callback [line 23] in the .then [line 30]
                resolve(successResult)
            } else {                                        //if success = false, reject() is called, setting the promise to "rejected"
                let failureResult = '"failure state"'       //anything passed into reject will be passed into the callback [line 25] in the .catch [line 32]
    ...
    const onFulfill = successResult => {
        console.log("~~~~~~~~   promise completion   ~~~~~~~~")
        console.log(successResult, 'Success!', nameOfYourPromise)
    }
    //this function is the callback for if the promise is unsuccessful.
    const onReject = failureResult => {
        console.log("~~~~~~~~   promise completion   ~~~~~~~~")
        console.log(failureResult, 'Failure.', nameOfYourPromise)
    }
    
    nameOfYourPromise                 //here's the cool part: promise.then and promise.catch
    .then(onFulfill)        //.then will run the function in the () when the promise's state is set to fulfilled, which happens when resolve() is called above. Here we use onFulfill, which is the function we wrote for the success case.
    .catch(onReject)        //.catch will run the function in the () when the promise's state is set to rejected, which happens when reject() is called above. Here we use onReject, which is the function we wrote for the failure case.
